...
if [[ -f .ai/CONTEXT.md ]]; then
  grep -Fq 'https://github.com/animine-ai/master-context' .ai/CONTEXT.md || { echo "Wrong context_repo" >&2; fail=1; }
  grep -Fq 'project_id: animetracker' .ai/CONTEXT.md || { echo "Wrong project_id" >&2; fail=1; }
  grep -Fq 'https://github.com/animine-ai/Anihyou' .ai/CONTEXT.md || { echo "Wrong source_repo" >&2; fail=1; }
else
