...
val RELEASE_MIGRATION_8_9 = object : Migration(8, 9) {
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL(
            """
            CREATE TABLE IF NOT EXISTS v3_external_mapping (
                mappingSubjectKey TEXT NOT NULL,
                seriesStableKey TEXT NOT NULL,
                ...
                parserVersion TEXT,
                PRIMARY KEY(mappingSubjectKey, externalProvider)
            )
            """.trimIndent(),
        )
        db.execSQL(
            """
            CREATE TABLE IF NOT EXISTS v3_mapping_attempt (
                attemptId INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
                mappingSubjectKey TEXT NOT NULL,
                seriesStableKey TEXT NOT NULL,
                siteSlug TEXT NOT NULL,
                subjectType TEXT NOT NULL,
        ...
        db.execSQL(
            "CREATE INDEX IF NOT EXISTS idx_v3_mapping_series_provider " +
                "ON v3_external_mapping(seriesStableKey, externalProvider)",
        )
        db.execSQL(
            "CREATE INDEX IF NOT EXISTS idx_v3_mapping_status_validation " +
                "ON v3_external_mapping(mappingStatus, validatedAt)",
        )
        db.execSQL(
            "CREATE INDEX IF NOT EXISTS idx_v3_attempt_subject_time " +
                "ON v3_mapping_attempt(mappingSubjectKey, externalProvider, attemptedAt)",
        )
        db.execSQL(
            "CREATE INDEX IF NOT EXISTS idx_v3_attempt_result_time " +
                "ON v3_mapping_attempt(resultKind, attemptedAt)",
        )
        db.execSQL(
            "INSERT OR REPLACE INTO schema_meta(key, schemaVersion, value, updatedAt) " +
