...
val RELEASE_MIGRATION_11_12 = object : Migration(11, 12) {
    override fun migrate(db: SupportSQLiteDatabase) {
        ...
        val newNames = listOf("v3_observation_cycle", "v3_cycle_source_observation",
            "v3_cycle_evidence_receipt", "v3_reconciliation_event", "v3_canonical_release_projection")
        newNames.forEach { name ->
            db.query("SELECT name FROM sqlite_master WHERE type = 'table' AND name = '$name'").use {
                check(!it.moveToNext()) { "unexpected v12 table in v11 database: $name" }
            }
        }
        ...
        db.execSQL("""CREATE TABLE IF NOT EXISTS v3_cycle_source_observation (
            ...
            target TEXT NOT NULL, track TEXT, negativeRequired INTEGER NOT NULL,
            result TEXT NOT NULL, health TEXT NOT NULL,
        ...
        db.execSQL("CREATE INDEX IF NOT EXISTS idx_v3_receipt_projection_cycle ON v3_cycle_evidence_receipt(projectionKey, cycleId)")
        db.execSQL("""CREATE TABLE IF NOT EXISTS v3_reconciliation_event (
            ...
            afterRevision INTEGER NOT NULL, payload TEXT NOT NULL,
            resolutionActor TEXT, resolutionReason TEXT, PRIMARY KEY(eventId))""")
